...
#
# WHAT THE CLIENT MUST SET UP IN THEIR GOAUTHENTIK:
#   1. Create an OAuth2 provider with:
#        - Redirect URI:  https://localhost/apps/user_oidc/code
#        - sub_mode:      user_username  (NOT the default hashed_user_id)
#   2. Create a Goauthentik Application linked to that provider.
#   3. Provide the generated client_id and client_secret.
#   4. Provide a test user account (username + password).
#N
